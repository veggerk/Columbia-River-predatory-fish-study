---
title: "walleye and northern pikeminnow mixing models with MixSIAR season effect"
...
iso_names=c("d15N","d13C"), factors="migration_timing",fac_random=FALSE,
...
save.image(file='walleye.model.with.season.RData')
```
...
# save model so that it can be called up again without having to be run again
save.image(file='npm.model.with.season.RData')
```

```{r npm jags code to modify factor plots, overall}
...
#        geom_density(alpha=.3) +
  ...
  ggplot2::theme(plot.margin = unit(c(0.2,0.5,0.2,0.2), "cm")))
...
```{r npm jags code to modify factor plots during migration}
#' Process mixing model output from JAGS
#'
#' \code{output_JAGS} processes the mixing model output, prints and saves (in the
#' working directory):
...
```{r npm jags code to modify factor plots after migration}
#' Process mixing model output from JAGS
#'
#' \code{output_JAGS} processes the mixing model output, prints and saves (in the
#' working directory):
...
if(!output_options[[3]]){   # if 'suppress posterior plots' is NOT checked
  ...
  if(n.effects >= 1 & mix$n.fe != 2){
    ...
    for(f1 in 1:mix$FAC[[1]]$levels){    # formerly factor1_levels
      ...
      print(ggplot2::ggplot(df, ggplot2::aes(x=x, fill=sources, colour=sources)) +
#        geom_density(alpha=.3) +
  ggplot2::geom_density(alpha=.3, ggplot2::aes(y=..scaled..), show.legend = FALSE) +
  ggplot2::xlim(0,1) +
  ggplot2::theme_classic() +
  ggplot2::scale_fill_brewer(palette = "Set1") +
  ggplot2::scale_colour_brewer(palette = "Set1") +
...
```{r walleye jags code to modify factor plots before migration}
#' Process mixing model output from JAGS
#'
#' \code{output_JAGS} processes the mixing model output, prints and saves (in the
#' working directory):
...
output_JAGS <- function(jags.1, mix, source, output_options=list(
                                                  ...
                                                  diag_name = "diagnostics",              # If yes, specify the base file name (.txt will be appended later)
                                                  indiv_effect = FALSE,                   # Is Individual a random effect in the model? (already specified)
                                                  plot_post_save_png = TRUE,             # Save posterior density plots as pngs?
                                                  plot_pairs_save_png = TRUE,            # Save pairs plot as png?
                                                  plot_xy_save_png = FALSE,
                                                  diag_save_ggmcmc = TRUE)){             # Save ggmcmc diagnostics as pdf?
mcmc.chains <- jags.1$BUGSoutput$n.chains
N <- mix$N
...
```{r walleye jags code to modify factor plots during migration}
#' Process mixing model output from JAGS
#'
#' \code{output_JAGS} processes the mixing model output, prints and saves (in the
#' working directory):
...
```{r walleye jags code to modify factor plots after migration}
#' Process mixing model output from JAGS
#'
#' \code{output_JAGS} processes the mixing model output, prints and saves (in the
#' working directory):
