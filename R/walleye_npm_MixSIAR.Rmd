---
title: "walleye and northern pikeminnow mixing models with MixSIAR"
...
save.image(file='walleye.model.no.effects.RData')
...
# save model so that it can be called up again without having to be run again
save.image(file='npm.model.no.effects.RData')
...
walleye output jags source code to change posterior plot
```{r walleye output jags source code to change posterior plot}
...
npm output jags source code to change posterior plot
```{r npn output jags source code to change posterior plot}
#' Process mixing model output from JAGS
#'
#' \code{output_JAGS} processes the mixing model output, prints and saves (in the
#' working directory):
